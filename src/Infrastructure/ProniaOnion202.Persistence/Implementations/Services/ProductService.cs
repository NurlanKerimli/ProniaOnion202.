...
namespace ProniaOnion202.Persistence.Implementations.Services
{
	internal class ProductService:IProductService
	{
		...
		{
			if (await _repository.IsExistAsync(p => p.Name == dto.Name)) throw new Exception("There is a product with the same name.");
			if (!await _categoryRepository.IsExistAsync(c => c.Id == dto.CategoryId)) throw new Exception("There is no such product.");
			...
			if(dto.ColorIds is not null)
			{
				...
				{
					if (!await _colorRepository.IsExistAsync(c => c.Id == colId)) throw new Exception("There is no such product.");
		...
		public async Task DeleteAsync(int id)
		{
			...
			if (existed is null) throw new Exception("There is no such product.");
